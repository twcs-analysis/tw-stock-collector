...
- **2026-02 月營收資料收集**（2026-03-09）
  ...
  - **公告率**: 46.2%（本日新增 299 檔）
  - **年增率正成長**: 538 檔 (59.9%)
  - **檔案位置**: `data/raw/revenue-daily/2026/2026-02.json`
  - **資料來源**: MOPS API
  - **亮點**:
    - 緯創 (3231): +165.25% YoY，營收 2,725.73 億（歷史新高）
    - 緯穎 (6669): +111.68% YoY，營收 946.33 億
    - 廣達 (2382): +52.30% YoY，營收 2,155.83 億

- **2026-02 月營收篩選報告**（2026-03-06）
  - 使用 ETF 持股篩選條件產生月營收分析報告
  - **檔案位置**: `analysis/reports/月營收篩選_2026-02_20260306_185932.md`
  - **篩選條件**: 年增率 ≥ 15%, 月增率 ≥ 0%, ETF 持有數 ≥ 1
  - **篩選結果**: 6 檔股票符合條件
  - **平均年增率**: 88.32%
  - **TOP 3**:
    - 緯創 (3231): +165.25% YoY（歷史新高）
    - 緯穎 (6669): +143.86% YoY
    - 廣達 (2382): +94.55% YoY

### 新增
- **2025 年完整逐字稿資料遷移**（2026-03-01）
  - 從 `~/yt-video/yt-moneyline` 複製 2025 年全年逐字稿至專案目錄
  - **資料範圍**: 2025-01-01 至 2025-12-31（252 個交易日）
  - **檔案數量**: 778 個逐字稿 TXT 檔案
  - **總大小**: 約 39MB
  - **檔案位置**: `data/transcripts/yt-moneyline/2025-*/`
  - **檔案格式**: 每日包含上集、中集、下集或全集（.txt 和 .mp3.txt 兩種版本）

- **moneyline-analysis Skill**（2026-03-01）
  - 新增錢線百分百逐字稿分析 skill
  - **檔案位置**: `~/.claude/skills/moneyline-analysis/SKILL.md`
  - **功能**:
    - 自動分析錢線百分百節目逐字稿，產生詳細投資分析報告
    - 支援單日分析和批次分析（日期範圍）
    - 使用獨立 Task agent 進行深度分析
    - 自動產生每日摘要報告
  - **報告架構**: 節目資訊、市場總體觀察、產業主題分析、技術分析、個股追蹤、投資策略、風險評估、行事曆、結論
  - **使用方式**: `/moneyline-analysis 2025-01-02` 或 `/moneyline-analysis 2025-01-02 2025-01-10`

- **2025-01-02 錢線百分百分析報告**（2026-03-01）
  - 首次使用 moneyline-analysis skill 產生的完整分析報告
  - **檔案位置**: `data/transcripts/yt-moneyline/2025-01-02/`
  - **內容**:
    - `moneyline_20250102_part1_analysis.md` - 上集分析（20K）
    - `moneyline_20250102_part2_analysis.md` - 中集分析（22K）
    - `moneyline_20250102_part3_analysis.md` - 下集分析（22K）
    - `daily_summary.md` - 每日摘要（16K）
  - **主要議題**:
    - 台股：開紅盤跌破季線、台積電法說會倒數、網通股低位階機會
    - 產業主題：航運 Q1 利多、機器人 CES 題材、重電強韌電網、無人機國家隊
    - 投資策略：葵國老師 2025 選股秘笈、丁建仁解套策略

### 修改
- **2026-02-27 錢線百分百分析報告更新**（2026-03-01）
  - 更新 `moneyline_20260227_part1_analysis.md`（從 655 行縮減為 288 行）
  - 刪除 `moneyline_20260227_part2_analysis.md`、`moneyline_20260227_part3_analysis.md`
  - 更新 `daily_summary.md`（從 397 行縮減為 107 行）
  - 原因：2026-02-27 為週五特別全集節目（非一般三集格式），重新整理為全集分析

### 刪除
- **清理 2026-02-07 舊檔案**（2026-03-01）
  - 刪除 HTML/PDF 導出檔案（已不再使用）
  - 刪除原始 TXT 逐字稿（已備份至 ~/yt-video）
  - 保留 Markdown 分析報告
...
- **刪除春節連假期間的錯誤資料**（2026-02-23）
  ...
  - **問題原因**: GitHub Actions 在春節連假（台股封關）期間仍執行資料收集
  - **錯誤特徵**:
    - 檔案大小異常（247KB vs 正常 607KB）
    - 僅包含上櫃股票（878 筆），缺少上市股票（1,080 筆）
  - **資料庫狀態**: 已確認資料庫中無這些日期的資料
  - **後續改進**: 加強交易日判斷邏輯、資料完整性驗證

### 資料更新
- **2026-02-23 每日交易資料收集**（2026-02-23）
  - 執行完整 Data Pipeline（收集 → 匯入 → 轉換）
  - **收集結果**:
  ...
  - **技術分析**: 878 檔股票（30 個指標）
  - **重點股票**:
    - 台積電 (2330): 收盤 1,900.00 (+15.0)，成交量 7,411 萬股
    - 櫻花建 (2539): 收盤 49.55 (+1.0)，成交量 152 萬股
  - **注意**: Git LFS 配額限制，尚未推送至遠端

- **2026-02-10 每日交易資料收集**（2026-02-10）
  - 執行完整 Data Pipeline（收集 → 匯入 → 轉換 → Git 提交）
  - **收集結果**:
    - price: 1,958 筆（上市 1,080 + 上櫃 878）
    - institutional: 1,837 筆（三大法人）
    - margin: 1,822 筆（融資融券）
    - top20_volume: 20 筆
    - lending: 無資料
  - **資料庫匯入**: 5,637 筆記錄
  - **技術分析**: 878 檔股票（30 個指標）
  - **重點股票**:
    - 台積電 (2330): 收盤 1,880.00 (+65.0)，成交量 4,567 萬股
    - 櫻花建 (2539): 收盤 48.50 (+0.8)，成交量 100 萬股

- **2026-01 月營收資料更新**（2026-02-10）
  - 執行 Revenue Pipeline 完整收集（Monthly 模式）
  - **本次收集**: 1,916 檔（完整資料）
  - **資料檔案**: `data/raw/revenue-daily/2026/2026-01.json`（952 KB，更新自 872 KB）
  - **收集模式**: Monthly 模式（完整資料模式）
  - **執行時間**: 約 2 秒（API 一次返回）
  - **重點發現**:
    - 整體市場表現強勁：69.2% 公司年增率正成長
    - AI 伺服器產業鏈持續成長（緯創 +151.54%、廣達 +61.92%、文曄 +151.67%）
    - 台積電營收創新高（4,012.55 億元，YoY +36.81%）
    - 鴻海營收 7,300.39 億元（YoY +35.53%）
  - **資料庫**: 已匯入 `stock_revenues` 表（1,916 筆記錄）

### 新增
- **2026-01 月營收篩選報告**（2026-02-10）
  - 新增嚴格版與寬鬆版篩選報告（支援 Markdown + PDF 雙格式）
  - 輸出位置：`analysis/reports/ETF持股篩選/2026-02-10/`
  - **嚴格版**（連續 3 個月 YoY 遞增）：
    - 符合條件股票數：18 檔
    - 創新高數量：11 檔（61.1%）
    - 平均年增率：206.01%
    - 平均月增率：17.98%
    - 代表股票：創見 (+440.61%)、志聖 (+124.10%)、華邦電 (+94.16%)、亞德客-KY (+66.81%)
  - **寬鬆版**（近三個月任意兩個月連續 YoY 遞增）：
    - 符合條件股票數：37 檔
    - 創新高數量：23 檔（62.2%）
    - 平均年增率：131.12%
    - 平均月增率：23.56%
    - 包含嚴格版所有股票 + 19 檔潛力股（如群創、文曄、景碩等）
  - **報告特色**：
    - ETF 持股認可度分級（風險評估）
    - 股價位階標示（歷史新高 vs 普通）
    - 成長趨勢標記（✓ 強勢、△ 改善、▽ 減緩）
    - 完整 ETF 持股清單
  - **生成檔案**：
    - `月營收報告_2026-01_嚴格版.md`（3.4K）
    - `月營收報告_2026-01_嚴格版.pdf`（583K）
    - `月營收報告_2026-01_寬鬆版.md`（5.4K）
    - `月營收報告_2026-01_寬鬆版.pdf`（715K）
