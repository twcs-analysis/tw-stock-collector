...
**個股期貨資料處理**（2026-03-15 新增）：
- ✅ **資料來源**: TAIFEX OpenAPI（台灣期貨交易所）
- ✅ **收集範圍**: 所有個股期貨日交易資料（~154 筆合約/日）
- ✅ **平行化回補**: 支援多執行緒同時收集（預設 3 個執行緒）
- ✅ **自動跳過非交易日**: 整合交易日曆服務
- ✅ **支援中斷續傳**: 自動跳過已收集檔案
- 📂 **儲存路徑**: `data/raw/stock_futures/YYYY/MM/YYYY-MM-DD.json`
- ⚡ **效能**: 0.14 秒/天（5 個執行緒，243 天資料僅需 34.3 秒）
- 📖 **文檔**: 參考 [STOCK_FUTURES_COLLECTION.md](docs/STOCK_FUTURES_COLLECTION.md)
...
7. **stock_futures** - 個股期貨每日交易資料（開高低收、成交量、未平倉）
