-- ============================================================================
-- 📊 月營收成長股 + ETF 持股篩選（Markdown 報告版 v3 - 寬鬆版）
-- ============================================================================
--
-- v3 寬鬆版改進：
-- - 趨勢條件放寬：近三個月中「任意兩個月連續」YoY 遞增即可
-- - 從資料庫讀取 ETF 持股資料
...
filtered_stocks AS (
    ...
    WHERE sr.year_month = :target_month
        ...
        -- 寬鬆趨勢條件：任意兩個月連續 YoY 遞增即可
        AND (
            -- 情況1: 最近兩個月連續遞增 (當月 > 上月 > 上上月 中的任意兩個連續)
            (tc.yoy_change_pct > tc.yoy_1m_ago AND tc.yoy_1m_ago > tc.yoy_2m_ago) OR
            -- 情況2: 當月和上月連續遞增
            (tc.yoy_change_pct > tc.yoy_1m_ago) OR
            -- 情況3: 上月和上上月連續遞增
            (tc.yoy_1m_ago > tc.yoy_2m_ago)
        )
...
SELECT
    ...
    '- 最低月增率: 0%' || E'\n' ||
    '- 最少 ETF 持有數: 1 個' || E'\n' ||
    '- 趨勢條件: 近三個月中任意兩個月連續 YoY 遞增（寬鬆版）' || E'\n' ||
