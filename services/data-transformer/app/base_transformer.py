"""
基礎轉換器類別

所有資料轉換器的抽象基礎類別，提供：
- 資料載入與儲存
- 轉換流程管理
- 驗證機制
- 效能監控
- 錯誤處理
"""

import time
from abc import ABC, abstractmethod
from datetime import datetime
from typing import Any, Dict, List, Optional, Union
from pathlib import Path
import pandas as pd

import sys
sys.path.insert(0, str(Path(__file__).parent.parent.parent))

from common.utils import (
    get_logger,
    get_global_config,
    FileHandler,
    DataValidator,
    build_file_path
)

logger = get_logger(__name__)


class TransformerError(Exception):
    """轉換器錯誤"""
    pass


class BaseTransformer(ABC):
    """
    基礎轉換器抽象類別

    所有具體轉換器都應繼承此類別並實作 transform() 方法
    """

    def __init__(
        self,
        config: Optional[Any] = None
    ):
        """
        Args:
            config: 配置實例
        """
        if config is None:
            config = get_global_config()

        self.config = config
        self.file_handler = FileHandler(config)
        self.validator = DataValidator(config)

        # Logger
        self.logger = get_logger(self.__class__.__name__)

        # 轉換資料的輸出路徑（與原始資料分開）
        self.output_base_path = 'data/transformed'

        # 統計資訊
        self.stats = {
            'total_records': 0,
            'success_count': 0,
            'failed_count': 0,
            'start_time': None,
            'end_time': None
        }

    @abstractmethod
    def transform(
        self,
        date: Union[str, datetime],
        stock_id: Optional[str] = None,
        **kwargs
    ) -> pd.DataFrame:
        """
        轉換資料 (抽象方法，子類必須實作)

        Args:
            date: 轉換日期
            stock_id: 股票代碼 (選用)
            **kwargs: 其他參數

        Returns:
            pd.DataFrame: 轉換後的資料

        Raises:
            TransformerError: 轉換失敗
        """
        pass

    @abstractmethod
    def get_source_data_type(self) -> str:
        """
        返回來源資料類型名稱

        Returns:
            str: 來源資料類型 (如 'price')
        """
        pass

    @abstractmethod
    def get_target_data_type(self) -> str:
        """
        返回目標資料類型名稱

        Returns:
            str: 目標資料類型 (如 'technical_analysis')
        """
        pass

    def load_source_data(
        self,
        date: Union[str, datetime],
        stock_id: Optional[str] = None
    ) -> pd.DataFrame:
        """
        載入來源資料

        Args:
            date: 日期
            stock_id: 股票代碼 (選用)

        Returns:
            pd.DataFrame: 來源資料
        """
        storage_config = self.config.storage
        file_path = build_file_path(
            base_path=storage_config.base_path,
            data_type=self.get_source_data_type(),
            date=date,
            stock_id=stock_id,
            file_format=storage_config.file_format,
            structure=storage_config.directory_structure
        )

        df = self.file_handler.load_dataframe(
            file_path=file_path,
            format=storage_config.file_format
        )

        if df is None or df.empty:
            self.logger.warning(f"來源資料為空: {file_path}")
            return pd.DataFrame()

        self.logger.info(f"載入來源資料: {file_path} ({len(df)} 筆)")
        return df

    def save_data(
        self,
        df: pd.DataFrame,
        date: Union[str, datetime],
        stock_id: Optional[str] = None,
        validate: bool = True
    ) -> bool:
        """
        儲存轉換後的資料

        Args:
            df: 要儲存的 DataFrame
            date: 日期
            stock_id: 股票代碼 (選用)
            validate: 是否驗證資料

        Returns:
            bool: 是否成功
        """
        if df is None or df.empty:
            self.logger.warning("資料為空，跳過儲存")
            return False

        # 驗證資料
        if validate:
            try:
                data_type = self.get_target_data_type()
                if not self.validator.validate(df, data_type, raise_on_error=False):
                    self.logger.warning(f"資料驗證失敗: {data_type}")
                    if self.config.validation.on_validation_error == 'raise':
                        return False
            except Exception as e:
                self.logger.error(f"驗證過程發生錯誤: {e}")

        # 建立檔案路徑 (使用 transformed 目錄而非 raw)
        storage_config = self.config.storage
        file_path = build_file_path(
            base_path=self.output_base_path,  # 使用轉換資料的輸出路徑
            data_type=self.get_target_data_type(),
            date=date,
            stock_id=stock_id,
            file_format=storage_config.file_format,
            structure=storage_config.directory_structure
        )

        # 決定合併模式
        merge_mode = 'merge_by_key' if storage_config.directory_structure == 'aggregate' else 'overwrite'

        # 儲存
        success = self.file_handler.save_dataframe(
            df=df,
            file_path=file_path,
            format=storage_config.file_format,
            merge_mode=merge_mode,
            create_backup=storage_config.options.create_backup
        )

        if success:
            self.stats['success_count'] += 1
            self.stats['total_records'] += len(df)
            self.logger.info(f"資料已儲存: {file_path} ({len(df)} 筆)")
        else:
            self.stats['failed_count'] += 1
            self.logger.error(f"資料儲存失敗: {file_path}")

        return success

    def transform_and_save(
        self,
        date: Union[str, datetime],
        stock_id: Optional[str] = None,
        **kwargs
    ) -> bool:
        """
        轉換並儲存資料 (便利方法)

        Args:
            date: 日期
            stock_id: 股票代碼
            **kwargs: 其他參數

        Returns:
            bool: 是否成功
        """
        try:
            # 轉換
            df = self.transform(date, stock_id, **kwargs)

            # 如果是空資料，記錄訊息但不算失敗
            if df is None or df.empty:
                self.logger.info(
                    f"轉換結果無資料: date={date}, stock_id={stock_id}"
                )
                return True  # 無資料不算失敗

            # 儲存
            return self.save_data(df, date, stock_id)

        except Exception as e:
            self.logger.error(
                f"轉換或儲存失敗: date={date}, stock_id={stock_id}, 錯誤={e}",
                exc_info=True
            )
            self.stats['failed_count'] += 1
            return False

    def batch_transform(
        self,
        dates: List[Union[str, datetime]],
        stock_ids: Optional[List[str]] = None,
        **kwargs
    ) -> Dict[str, Any]:
        """
        批次轉換資料

        Args:
            dates: 日期列表
            stock_ids: 股票代碼列表 (選用)
            **kwargs: 其他參數

        Returns:
            Dict: 轉換統計資訊
        """
        self.stats['start_time'] = datetime.now()
        self.logger.info(
            f"開始批次轉換: {len(dates)} 個日期, "
            f"{len(stock_ids) if stock_ids else 0} 檔股票"
        )

        success_count = 0
        failed_count = 0

        # 根據是否需要股票代碼來決定轉換方式
        if stock_ids:
            # 需要逐一轉換每檔股票
            for date in dates:
                for stock_id in stock_ids:
                    try:
                        if self.transform_and_save(date, stock_id, **kwargs):
                            success_count += 1
                        else:
                            failed_count += 1
                    except Exception as e:
                        self.logger.error(
                            f"批次轉換失敗: {date}, {stock_id}, {e}"
                        )
                        failed_count += 1
        else:
            # 不需要股票代碼 (批次轉換)
            for date in dates:
                try:
                    if self.transform_and_save(date, stock_id=None, **kwargs):
                        success_count += 1
                    else:
                        failed_count += 1
                except Exception as e:
                    self.logger.error(f"批次轉換失敗: {date}, {e}")
                    failed_count += 1

        self.stats['end_time'] = datetime.now()
        duration = (self.stats['end_time'] - self.stats['start_time']).total_seconds()

        summary = {
            'success_count': success_count,
            'failed_count': failed_count,
            'total_records': self.stats['total_records'],
            'duration_seconds': duration,
            'records_per_second': self.stats['total_records'] / duration if duration > 0 else 0
        }

        self.logger.info(
            f"批次轉換完成: 成功 {success_count}, 失敗 {failed_count}, "
            f"耗時 {duration:.2f}s"
        )

        return summary

    def get_stats(self) -> Dict[str, Any]:
        """
        獲取統計資訊

        Returns:
            Dict: 統計資訊
        """
        return self.stats.copy()

    def reset_stats(self) -> None:
        """重置統計資訊"""
        self.stats = {
            'total_records': 0,
            'success_count': 0,
            'failed_count': 0,
            'start_time': None,
            'end_time': None
        }

    def _format_date(self, date: Union[str, datetime]) -> str:
        """
        格式化日期為 YYYY-MM-DD

        Args:
            date: 日期

        Returns:
            str: 格式化後的日期
        """
        if isinstance(date, str):
            return date
        elif isinstance(date, datetime):
            return date.strftime('%Y-%m-%d')
        else:
            raise ValueError(f"不支援的日期格式: {type(date)}")

    def __repr__(self) -> str:
        return (
            f"{self.__class__.__name__}("
            f"source={self.get_source_data_type()}, "
            f"target={self.get_target_data_type()})"
        )
